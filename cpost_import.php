...
// Assuming the CSV file is in the same directory and named 'data.csv'.
$csvFile = fopen('954.csv', 'r');
...
if ($csvFile) {
    while (($line = fgetcsv($csvFile)) !== FALSE) {
        ...
        $tel_cislo = $columns[1];
        $kod_adresy = $columns[5];
        $error = 0;

        // Prepare the SQL query
        $query19 = "INSERT INTO cpost (tel_cislo, adresa) VALUES ('$tel_cislo', '$kod_adresy');";
...
$query25 = "SELECT tel_cislo, adresa FROM cpost LIMIT 1;";
if ($result25 = mysqli_query($link, $query25)) {
    while ($row25 = mysqli_fetch_row($result25)) {
        $tel_cislo = $row25[0];
        $kod_adresy = $row25[1];

        $query15 = "SELECT nazev_ulice, cislo_popisne, cislo_orientacni, nazev_obce, nazev_casti_obce, nazev_okresu, longitude, latitude, kod_objektu, kod_adresy, kod_obce, kod_casti_obce, kod_ulice FROM stanice WHERE tel_cislo = '$tel_cislo';";
        if ($result15 = mysqli_query($link, $query15)) {
            if (mysqli_num_rows($result15) > 0) {
                while ($row15 = mysqli_fetch_row($result15)) {
                    ...
                    if ($kod_adresy != '') {
                        ...
                        if ($new_nazev_obce == "Praha") {
                            $new_nazev_okresu = "Praha";
                        }
            ...
            if ($error == 0) {
                ...
                $query150 = "DELETE FROM cpost WHERE tel_cislo = $tel_cislo;";
                $prikaz150 = mysqli_query($link, $query150);
            ...
            if ($error == 0) {
                $query143 = "INSERT INTO stanice (`prijmeni`,`jmeno`,`tel_cislo`,`ico`,`nazev_ulice`,`cislo_popisne`,`cislo_orientacni`,`cislo_podlazi`,`cislo_bytu`,`nazev_obce`,`nazev_casti_obce`,`nazev_okresu`,`longitude`,`latitude`,`kod_objektu`,`kod_adresy`,`kod_obce`,`kod_casti_obce`,`kod_ulice`,`OpID`) VALUES ('$prijmeni','','$tel_cislo','47114983','$new_nazev_ulice','$new_cislo_popisne','$new_cislo_orientacni','','','$new_nazev_obce','$new_nazev_casti_obce','$new_nazev_okresu','$new_longitude','$new_latitude','$new_kod_objektu','$kod_adresy','$new_kod_obce','$new_kod_casti_obce','$new_kod_ulice','444');";
                echo $query143;
                $prikaz143 = mysqli_query($link, $query143);
